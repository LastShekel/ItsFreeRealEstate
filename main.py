...
def _get_town(pivot: pd.DataFrame,
              column: str,
              saleamount_top: int) -> List[str]:
    return list(pivot.iloc[pivot[column].argsort()[-saleamount_top:]].index)


def get_top_saleamount(data_df: pd.DataFrame,
                       saleamount_top: int = 10) -> Dict[int, List[str]]:
    ...
    result = {
        int(column): _get_town(pivot, column, saleamount_top)
...
def main() -> None:
    ...
    if saleamount_top:
        ...
        tops = get_top_saleamount(data_df, saleamount_top)
        print('%s towns that have the highest '
